...
pub fn read_body_as_string(
    ...
    return body.fold(Vec::new(), |mut vector, chunk| {
        ...
        .then(move |body_as_byte_vector| {
            match body_as_byte_vector {
                Ok(byte_vector) => {
...
mod tests {
    ...
    fn test_read_body_as_string() {
        ...
        let what_is_read_from_body = read_body_as_string(body_composed)
    ...
    fn test_read_response_body_as_string_with_header() {
        ...
        assert_eq!(header_map_read.get(VALID_HEADER_KEY.clone())
    ...
    fn test_read_response_body_as_string_with_invalid_header() {
        ...
        assert_eq!(what_is_read_from_body, RANDOM_STRING.clone());
        let header_map_read = what_is_read_from_response.header_map;
        assert_eq!(header_map_read.contains_key(INVALID_HEADER_KEY.clone()), false);
    }
    ...
    fn mock_setup_server() {
        ...
        let new_service = move || {
            service_fn_ok(|_| {
                ...
                response.headers_mut().insert(HeaderName::from_str(VALID_HEADER_KEY.as_str())
        ...
        let new_service = move || {
            service_fn_ok(|_| {
                ...
                *response.status_mut() = StatusCode::from_u16(400)
                    .expect("Error reading status code from integer");
                response
            })
        };
        let server = Server::bind(&socket_addr)
