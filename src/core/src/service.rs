...
impl Poet2Service {
    ...
    pub fn finalize_block(&mut self, consensus: &[u8]) -> BlockId {
    ...
    pub fn get_wait_time(
        ...
        info!(
            "Getting new wait time for block num {}",
            pre_chain_head.block_num
        );
        debug!(
            "Getting new wait time for block id {} prev_id {}",
            poet2_util::to_hex_string(&pre_chain_head.block_id),
            poet2_util::to_hex_string(&pre_chain_head.previous_id)
        );
        if pre_chain_head.block_num != 0_u64 {
            // non-genesis block
            let result = poet2_util::payload_to_wc_and_sig(&pre_chain_head.payload);
        ...
        let mut wait_time = minimum_duration - local_mean * tagd.log10();
    ...
    pub fn get_settings(
        ...
        self.service.get_settings(block_id, keys)
    }

    pub fn get_setting(&mut self, block_id: BlockId, key: &str) -> String {
        let settings_result = self.service.get_settings(block_id, vec![key.to_string()]);
    ...
    pub fn get_setting_from_head(&mut self, key: &str) -> String {
    ...
    pub fn get_state(
        ...
        self.service.get_state(block_id, vec![key.to_string()])
    }
    ...
    pub fn verify_wait_certificate(
        ...
        let sig_verify_status = EnclaveConfig::verify_wait_certificate(
            self.enclave.enclave_id,
            &poet_pub_key,
            &wait_cert,
            &wait_cert_sign,
        ...
        debug!(
            "Prev ID {}, Block Num {}, Block Summary {}, Validator ID {:?} from WC {:?}",
            prev_id.clone(),
            block.block_num,
            summary.clone(),
            signer_id.clone(),
...
mod tests {
    ...
    fn test_service() {
        ...
        let svc_thread = ::std::thread::spawn(move || {
            ...
            let mut zmq_svc = ZmqService::new(sender, ::std::time::Duration::from_secs(10));
